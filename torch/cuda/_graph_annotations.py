...
def _check_cuda(result: Any) -> Any:
    ...
    if err != _cuda_runtime.cudaError_t.cudaSuccess:
        _, err_str = _cuda_runtime.cudaGetErrorString(err)
...
def _get_tools_id(node: Any) -> int | None:
    ...
    if _tools_id_available is None:
        try:
            tools_id = _check_cuda(_cuda_runtime.cudaGraphNodeGetToolsId(node))
        except Exception:
        ...
        return tools_id
    return _check_cuda(_cuda_runtime.cudaGraphNodeGetToolsId(node))
...
def _get_capture_graph(stream: Any) -> Any:
    ...
    status, _id, graph, _deps, _edge_data, _num_deps = _check_cuda(
        _cuda_runtime.cudaStreamGetCaptureInfo(stream)
    )
    if (
        status
        != _cuda_runtime.cudaStreamCaptureStatus.cudaStreamCaptureStatusActive
    ):
        return None
    return graph
...
def _get_node_count(graph: Any) -> int:
    ...
    _, num = _check_cuda(_cuda_runtime.cudaGraphGetNodes(graph, numNodes=0))
    return num
...
def _get_annotatable_types() -> set[Any]:
    ...
    if _ANNOTATABLE_TYPES is None:
        _ANNOTATABLE_TYPES = {
            _cuda_runtime.cudaGraphNodeType.cudaGraphNodeTypeKernel,
            _cuda_runtime.cudaGraphNodeType.cudaGraphNodeTypeMemcpy,
        }
...
def mark_kernels(annotation: Any):
    ...
    stream = _cuda_runtime.cudaStream_t(
...
def resolve_pending_annotations() -> None:
    ...
    try:
        ...
        nodes, num = _check_cuda(
            _cuda_runtime.cudaGraphGetNodes(graph, numNodes=num)
        )
        ...
        for i in range(num):
            ...
            while (
                scope_ptr < len(sorted_scopes)
                and sorted_scopes[scope_ptr][1] <= i
            ):
            ...
            node = nodes[i]
            node_type = _check_cuda(_cuda_runtime.cudaGraphNodeGetType(node))
...
def remap_to_exec_graph(torch_cuda_graph: torch.cuda.CUDAGraph) -> None:
    ...
    exec_graph_id = _check_cuda(_cuda_runtime.cudaGraphExecGetId(exec_handle))
